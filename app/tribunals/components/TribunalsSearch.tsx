import { Search, ArrowRight, RefreshCw, Landmark, Briefcase, MapPin, Activity } from "lucide-react";

export default function TribunalsSearch() {
  const tabs = [
    { label: "BROWSE TRIBUNALS", active: true },
    { label: "SEARCH CASES / ORDERS" },
    { label: "ADVANCED SEARCH" },
  ];

  const dropdowns = [
    { label: "Tribunal Type", icon: <Landmark size={14} /> },
    { label: "Ministry / Department", icon: <Briefcase size={14} /> },
    { label: "State / Region", icon: <MapPin size={14} /> },
    { label: "Status", icon: <Activity size={14} /> },
  ];

  return (
    <section className="relative z-20 max-w-[1280px] mx-auto px-4 mt-8 mb-12">
      <div className="bg-[#0d1b3e] rounded-xl border border-[#c9a84c]/20 shadow-xl overflow-hidden">
        
        {/* Tabs */}
        <div className="flex items-center overflow-x-auto border-b border-white/10 hide-scrollbar">
          {tabs.map((tab, idx) => (
            <button 
              key={idx}
              className={`flex items-center justify-center px-8 py-5 text-[12px] font-bold tracking-wider uppercase whitespace-nowrap transition-colors ${
                tab.active 
                  ? "text-[#c9a84c] border-b-2 border-[#c9a84c] bg-white/5" 
                  : "text-white/80 hover:bg-white/5 hover:text-white"
              }`}
            >
              {tab.label}
            </button>
          ))}
        </div>

        <div className="p-6">
          {/* Main Search Input */}
          <div className="flex bg-white rounded-md overflow-hidden mb-6 h-[50px] shadow-sm">
            <div className="pl-4 pr-3 flex items-center justify-center text-[#9ca3af]">
              <Search size={18} strokeWidth={2} />
            </div>
            <input 
              type="text" 
              placeholder="Search tribunals, benches, jurisdiction, keywords..." 
              className="flex-1 text-[13px] outline-none text-[#374151] placeholder-[#9ca3af]"
            />
            <button className="flex items-center gap-2 bg-[#c9a84c] text-[#0d1b3e] px-8 font-bold text-[13px] uppercase tracking-wider hover:bg-[#d4a93a] transition-colors">
              SEARCH <ArrowRight size={14} strokeWidth={2.5} />
            </button>
          </div>

          {/* Filters Row */}
          <div className="flex flex-col md:flex-row items-center gap-4">
            {dropdowns.map((dropdown, idx) => (
              <div key={idx} className="relative flex-1 w-full">
                <div className="absolute left-3 top-1/2 -translate-y-1/2 text-white/50 pointer-events-none">
                  {dropdown.icon}
                </div>
                <select
                  aria-label={dropdown.label}
                  className="w-full text-[12px] text-white/90 bg-transparent border border-white/20 rounded-md pl-9 pr-8 py-2.5 cursor-pointer hover:border-[#c9a84c]/50 transition-colors outline-none focus:border-[#c9a84c] appearance-none"
                >
                  <option value="" className="text-[#0d1b3e]">{dropdown.label}</option>
                </select>
                <div className="absolute right-3 top-1/2 -translate-y-1/2 text-white/50 pointer-events-none">
                  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M6 9l6 6 6-6"/></svg>
                </div>
              </div>
            ))}
            
            <button className="flex items-center gap-2 text-[#c9a84c] hover:text-white text-[12px] font-semibold tracking-wider px-2 transition-colors flex-shrink-0">
              Reset <RefreshCw size={12} />
            </button>
          </div>
        </div>

      </div>
    </section>
  );
}

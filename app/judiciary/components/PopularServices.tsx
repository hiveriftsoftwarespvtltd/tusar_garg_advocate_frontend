      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
        {services.map((service, idx) => (
          <Link 
            key={idx} 
            href={service.link}
            className="bg-white border border-[#e8ebf2] rounded-xl p-5 flex flex-col items-center text-center group cursor-pointer hover:border-[#c9a84c]/50 hover:shadow-md transition-all duration-300 h-full min-h-[220px]"
          >
            <div className="text-[#0d1b3e] mb-4 group-hover:scale-110 group-hover:text-[#c9a84c] transition-all duration-300">
              {service.icon}
            </div>
            <h3 className="text-[12px] font-semibold text-[#0d1b3e] mb-2 leading-snug group-hover:text-[#c9a84c] transition-colors flex-shrink-0">
              {service.title}
            </h3>

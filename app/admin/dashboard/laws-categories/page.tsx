...
export default function AdminLawsCategories() {
  ...
  const loadData = async () => {
    try {
      ...
      console.error("Failed to load laws categories", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editId) {
        await fetchApi(`/laws-categories/${editId}`, {
          method: "PUT",
        ...
        await fetchApi('/laws-categories', {
          method: "POST",
          body: JSON.stringify(formData)
        });
        Swal.fire({
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      try {
        await fetchApi(`/laws-categories/${id}`, { method: "DELETE" });
        Swal.fire({
          title: 'Deleted!',
  ...
  const filteredCategories = useMemo(() => {
    ...
    return categories.filter((cat) => {
      return (
        ...
        cat.acts?.toLowerCase().includes(query) ||
        cat.sections?.toLowerCase().includes(query) ||
  ...
  }, [sortedCategories, startIndex, pageSize]);

  // Adjust page if current page becomes out of range
  useEffect(() => {
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[80vh] overflow-y-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input 
                    type="text" 
                    placeholder="e.g. Apex Court & Writs"
                    className="w-full p-2.5 border border-gray-300 rounded-lg text-sm text-black outline-none focus:border-[#c9a84c]"
                    value={formData.tag} 
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Acts Count</label>
                  <input 
                    type="text" 
                    placeholder="e.g. 18 Acts"
                    className="w-full p-2.5 border border-gray-300 rounded-lg text-sm text-black outline-none focus:border-[#c9a84c]"
                    value={formData.acts} 
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Sections Count</label>
                  <input 
                    type="text" 
                    placeholder="e.g. 245 Sections"
                    className="w-full p-2.5 border border-gray-300 rounded-lg text-sm text-black outline-none focus:border-[#c9a84c]"
                ...
                <div>
                  <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Upload Card Image</label>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Image URL (or Base64)</label>
                <input 
                  type="text" 
                  placeholder="https://..."
                  className="w-full p-2.5 border border-gray-300 rounded-lg text-sm text-black outline-none focus:border-[#c9a84c]"
              ...
              <div>
                <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Short Description / Summary</label>
                <textarea 
                  rows={3} 
                  placeholder="e.g. Writ Petitions, Fundamental Rights & Supreme Court Litigation"
                  className="w-full p-2.5 border border-gray-300 rounded-lg text-sm text-black outline-none focus:border-[#c9a84c]"
                  value={formData.desc} 
              ...
              <div className="flex items-center gap-2 pt-1">
                ...
                <label htmlFor="isFeatured" className="text-sm font-bold text-[#0d1b3e]">
                  Featured (Display on Homepage & Laws Page)
                </label>
              </div>
              ...
              <div className="flex justify-end gap-3 pt-4 border-t border-gray-100 mt-4">
                ...
                >
                  {editId ? "Update Category" : "Save New Category"}
                </button>
              </div>
            </form>
          </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center min-h-[300px]">
            <div className="flex items-center gap-3 text-gray-500 font-medium">
              <RefreshCw className="animate-spin" size={20} /> Loading Law Categories...
            </div>
          </div>
        ) : paginatedCategories.length === 0 ? (
          <div className="p-12 text-center text-gray-500 space-y-2">
            <p className="text-base font-bold text-[#0d1b3e]">No matching categories found.</p>
            <p className="text-xs text-gray-400">Try adjusting your search query or add a new category.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th 
                    onClick={() => handleSort("acts")}
                    className="p-4 font-bold text-xs uppercase tracking-wider text-[#0d1b3e] cursor-pointer hover:bg-gray-100 transition-colors"
                  >
                    <div className="flex items-center gap-1.5">
                      <span>Acts & Sections</span>
                      {sortField === "acts" ? (
                        sortOrder === "asc" ? <ChevronUp size={14} className="text-[#c9a84c]" /> : <ChevronDown size={14} className="text-[#c9a84c]" />
                      ) : (
                        <ArrowUpDown size={13} className="text-gray-400" />
              ...
              <tbody className="divide-y divide-gray-100">
                {paginatedCategories.map((cat) => (
                  <tr key={cat._id} className="hover:bg-gray-50/80 transition-colors">
                    ...
                    <td className="p-4">
                      <p className="font-bold text-[#0d1b3e] text-sm">{cat.name}</p>
                      <span className="inline-block text-[10px] font-extrabold text-[#c9a84c] bg-[#0d1b3e] px-2 py-0.5 rounded mt-0.5">
                        {cat.tag || "General"}
                      </span>
                    </td>
                    <td className="p-4 text-xs text-gray-600">
                      <p className="font-bold text-gray-800">{cat.acts || "0 Acts"}</p>
                      <p className="text-gray-500">{cat.sections || "0 Sections"}</p>
                    </td>

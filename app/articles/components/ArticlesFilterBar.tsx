          ...
          <div className="flex items-center gap-3 w-full lg:w-[55%] overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            ...
            <div className="flex items-center gap-2 ml-2">
              {popularTopics.map((topic, idx) => (
                <button key={idx} className="border border-white/20 text-white text-[10px] md:text-[11px] px-3 md:px-4 py-1.5 rounded-md hover:bg-white/10 hover:border-white/40 transition-all whitespace-nowrap">
                  {topic}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 w-full gap-3 md:gap-4 flex-1">
            {dropdowns.map((label, idx) => (
              <div key={idx} className="relative w-full">
                <select
                  aria-label={label}
                  className="w-full text-[11px] md:text-[12px] text-white bg-transparent border border-white/20 rounded-md pl-3 pr-8 py-2.5 cursor-pointer hover:border-white/40 transition-colors outline-none focus:border-[#c9a84c] appearance-none"
                >

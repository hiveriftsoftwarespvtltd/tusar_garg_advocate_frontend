...
import { legalMaximsData, LegalMaximItem } from "../data/resourcesDetailData";

export default function MaximsClientView() {
  ...
  const filteredMaxims = legalMaximsData.filter((item) => {
    const matchesCat = selectedCategory === "All" || item.category === selectedCategory;
    const q = searchQuery.toLowerCase().trim();
    const matchesSearch =
      !q ||
      item.maxim.toLowerCase().includes(q) ||
      item.literalMeaning.toLowerCase().includes(q) ||
      item.jurisprudenceScope.toLowerCase().includes(q) ||
      item.statutoryEquivalent.toLowerCase().includes(q) ||
      item.landmarkPrecedents.some((p) => p.caseName.toLowerCase().includes(q));
      ...
      <section className="relative bg-gradient-to-b from-[#071126] via-[#0d1b3e] to-[#071126] text-white py-14 sm:py-16 px-4 sm:px-6 overflow-hidden border-b border-[#c9a84c]/20">
        ...
        <div className="max-w-[1300px] mx-auto relative z-10">
          <div className="flex flex-wrap items-center gap-2 text-[12px] font-medium text-gray-400 mb-6">
            ...
            <span>/</span>
            <span className="text-[#c9a84c] font-semibold">Legal Maxims</span>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            <div className="lg:col-span-8 space-y-4">
              <div className="inline-flex items-center gap-2 bg-[#c9a84c]/10 border border-[#c9a84c]/40 px-3.5 py-1 rounded-full text-[#c9a84c] text-[11px] font-bold tracking-wider uppercase shadow-sm">
                <Scale size={14} />
                <span>LATIN MAXIMS & STATUTORY JURISPRUDENCE</span>
              </div>
              ...
              <p className="text-gray-300 text-base sm:text-lg max-w-3xl leading-relaxed font-light">
                Authoritative compendium of Latin and judicial maxims governing Natural Justice, Criminal Intent, Civil Liability, and Procedural Jurisprudence with Supreme Court ratios.
              </p>
            ...
            <div className="lg:col-span-4">
              <div className="bg-white/[0.04] backdrop-blur-md border border-white/10 rounded-2xl p-5 sm:p-6 shadow-2xl space-y-3">
                <div className="text-[11px] uppercase font-bold tracking-widest text-[#c9a84c] border-b border-white/10 pb-2.5 flex items-center gap-2">
                  <ShieldCheck size={15} />
                  <span>JURISPRUDENTIAL WEIGHT</span>
                </div>
                <div className="grid grid-cols-2 gap-3 pt-1 text-xs">
                  <div className="bg-[#071126]/60 border border-white/5 rounded-xl p-3">
                    <span className="text-[10px] text-gray-400 block uppercase">Natural Justice</span>
                    <span className="text-sm font-bold text-white mt-1 block text-[#c9a84c]">Universal</span>
                    <span className="text-[10px] text-gray-400">Courts & Tribunals</span>
                  </div>
                  <div className="bg-[#071126]/60 border border-white/5 rounded-xl p-3">
                    <span className="text-[10px] text-gray-400 block uppercase">Criminal Intent</span>
                    <span className="text-sm font-bold text-white mt-1 block text-[#c9a84c]">Mens Rea</span>
                    <span className="text-[10px] text-gray-400">Core test of guilt</span>
                  </div>
                  <div className="bg-[#071126]/60 border border-white/5 rounded-xl p-3">
                    <span className="text-[10px] text-gray-400 block uppercase">Judicial Value</span>
                    <span className="text-sm font-bold text-white mt-1 block text-[#c9a84c]">Binding</span>
                    <span className="text-[10px] text-gray-400">Supreme Court ratio</span>
                  </div>
                  <div className="bg-[#071126]/60 border border-white/5 rounded-xl p-3">
                    <span className="text-[10px] text-gray-400 block uppercase">Language</span>
                    <span className="text-sm font-bold text-white mt-1 block text-[#c9a84c]">Latin Roots</span>
                    <span className="text-[10px] text-gray-400">English translation</span>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="bg-white border-b border-gray-200 sticky top-0 z-30 shadow-sm">
        <div className="max-w-[1300px] mx-auto px-4 sm:px-6 py-3 flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="relative w-full md:w-80">
            ...
            <input
              type="text"
              placeholder="Search legal maxim, latin phrase, case..."
              value={searchQuery}
      ...
      <main className="max-w-[1300px] mx-auto px-4 sm:px-6 py-10">
          ...
          {filteredMaxims.map((item, idx) => (
            ...
            >
              <div>
                ...
                <h3 className="font-serif text-lg font-bold text-[#0d1b3e] mb-1 leading-snug">
                  {item.maxim}
                </h3>

                <p className="text-xs font-semibold text-[#c9a84c] mb-3">
                  &ldquo;{item.literalMeaning}&rdquo;
                </p>

                <p className="text-xs sm:text-[13px] text-gray-700 mb-4 leading-relaxed">
                  {item.jurisprudenceScope}
                </p>

                <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 space-y-2.5 text-xs">
                  <div>
                    <strong className="text-gray-500 uppercase tracking-wider text-[10.5px] block mb-1">
                      Court Application:
                    </strong>
                    <span className="text-gray-700 leading-relaxed block">{item.applicationInCourts}</span>
                  </div>
                  ...
                  <div>
                    ...
                    <span className="font-semibold text-[#0d1b3e]">{item.statutoryEquivalent}</span>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1.5">
                      {item.landmarkPrecedents.map((prec, i) => (
                        <div key={i} className="text-xs">
                          <span className="font-bold text-[#c9a84c]">{prec.caseName}</span>
                          <p className="text-gray-600 text-[11.5px] leading-snug mt-0.5">{prec.ratio}</p>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        {filteredMaxims.length === 0 && (
          <div className="text-center py-12 bg-white rounded-2xl border border-gray-200 p-8">
            ...
            <p className="text-gray-600 text-sm font-semibold">No maxims found matching your search.</p>
            <button
              onClick={() => {
        ...
        <div className="mt-14 pt-8 border-t border-gray-200">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            >
              ...
              <p className="text-[10px] text-gray-500">Filing roadmaps & guides</p>
            </Link>
            ...
            >
              ...
              <p className="text-[10px] text-gray-500">Search landmark verdicts</p>
            </Link>
